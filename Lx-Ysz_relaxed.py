...
sz_data = pd.merge(master_file,offset, left_on = master_file['Cluster'].str.casefold(), right_on = offset['Cluster'].str.casefold(), how ='inner')
...
### Setting up Lx
Lx_r = r_clusters['Lx(1e44)']
Lx_new_r = Lx_r * (E)**(-5/3) #### The power of redshift evolution for this relations is -5/3
...
ycept_r,Norm_r,Slope_r,Scatter_r = general_functions.calculate_bestfit(log_Ysz_new_r,sigma_Ysz_r,log_Lx_r,sigma_Lx_r)
...
# bestfit_bootstrap.to_csv('Lx-Ysz_r_BCES.csv')
# =============================================================================

data = pd.read_csv('/home/schubham/Thesis/Thesis/Scaling_relations/bces_data/Lx-Ysz_r_BCES.csv')
...
# Setting up Lx
Lx_d = d_clusters['Lx(1e44)']
Lx_new_d = Lx_d * (E)**(-5/3) #### The power of redshift evolution for this relations is -5/3
...
ycept_d,Norm_d,Slope_d,Scatter_d = general_functions.calculate_bestfit(log_Ysz_new_d,sigma_Ysz_d,log_Lx_d,sigma_Lx_d)
...
# bestfit_bootstrap_dict = {'Normalization': best_A, 'Slope': best_B, 'Scatter': best_scatter}              
# bestfit_bootstrap = pd.DataFrame(bestfit_bootstrap_dict)
# bestfit_bootstrap.to_csv('Lx-Ysz_d_BCES.csv')
# =============================================================================

data = pd.read_csv('/home/schubham/Thesis/Thesis/Scaling_relations/bces_data/Lx-Ysz_d_BCES.csv')
...
z_r = general_functions.plot_bestfit(Y_linspace, 35, 1, ycept_r, Slope_r)
z_d = general_functions.plot_bestfit(Y_linspace, 35, 1, ycept_d, Slope_d)

plt.plot(Y_linspace,z_r, label = 'Best fit relaxed',color = 'black')
plt.plot(Y_linspace,z_d, label = 'Best fit disturbed',color = 'blue')
plt.errorbar(Ysz_r,Lx_r,xerr = err_Ysz_r,yerr=err_Lx_r,color = 'green',ls='',fmt='.', capsize = 2,alpha= 1, elinewidth = 0.6, label = f'relaxed clusters ({len(Lx_r)})' )
plt.errorbar(Ysz_d,Lx_d,xerr = err_Ysz_d,yerr=err_Lx_d,color = 'red',ls='',fmt='.', capsize = 2,alpha= 1, elinewidth = 0.6, label = f'disturbed clusters ({len(Lx_d)})' )
plt.xscale('log')
plt.yscale('log')
...
plt.savefig('/home/schubham/Thesis/Thesis/Plots/Relaxed-Disturbed_comparison/Lx-Ysz_rVd_bestfit.png',dpi=300,bbox_inches="tight")
plt.show()
...
err_Ysz_r = ((e_Y_arcmin_r)* (D_a.value**2) * (np.pi / (60*180))**2)
err_Lx_r = r_clusters['eL(%)']*Lx_new_r/100
ycept_r_Mcut,Norm_r_Mcut,Slope_r_Mcut,Scatter_r_Mcut = general_functions.calculate_bestfit(log_Ysz_new_r,sigma_Ysz_r,log_Lx_r,sigma_Lx_r)
...
# bestfit_bootstrap_dict = {'Normalization': best_A, 'Slope': best_B, 'Scatter': best_scatter}              
# bestfit_bootstrap = pd.DataFrame(bestfit_bootstrap_dict)
# bestfit_bootstrap.to_csv('Lx-Ysz_r(Mcut)_BCES.csv')
# =============================================================================

data = pd.read_csv('/home/schubham/Thesis/Thesis/Scaling_relations/bces_data/Lx-Ysz_r(Mcut)_BCES.csv')
...
err_Ysz_d = ((e_Y_arcmin_d)* (D_a.value**2) * (np.pi / (60*180))**2)
err_Lx_d = d_clusters['eL(%)']*Lx_new_d/100
ycept_d_Mcut,Norm_d_Mcut,Slope_d_Mcut,Scatter_d_Mcut = general_functions.calculate_bestfit(log_Ysz_new_d,sigma_Ysz_d,log_Lx_d,sigma_Lx_d)
...
# bestfit_bootstrap_dict = {'Normalization': best_A, 'Slope': best_B, 'Scatter': best_scatter}              
# bestfit_bootstrap = pd.DataFrame(bestfit_bootstrap_dict)
# bestfit_bootstrap.to_csv('Lx-Ysz_d(Mcut)_BCES.csv')
# 
# =============================================================================
data = pd.read_csv('/home/schubham/Thesis/Thesis/Scaling_relations/bces_data/Lx-Ysz_d(Mcut)_BCES.csv')
...
z_r = general_functions.plot_bestfit(Y_linspace, 35, 1, ycept_r_Mcut, Slope_r_Mcut)
z_d = general_functions.plot_bestfit(Y_linspace, 35, 1, ycept_d_Mcut, Slope_d_Mcut)
...
plt.ylabel(r'$L_{\mathrm{X}}*E(z)^{-5/3}\,(*10^{44} \,\mathrm{erg\,s^{-1})}$ ')
plt.xlabel(r' $Y_{\mathrm{SZ}}$ ($\mathrm{kpc}^{2}$)')
plt.title('$L_{\mathrm{X}}-Y_{\mathrm{SZ}}$ Best fit ($M_{\mathrm{cluster}} > 10^{14}\mathrm{M}_{\odot}$)')
plt.savefig('/home/schubham/Thesis/Thesis/Plots/Relaxed-Disturbed_comparison/Lx-Ysz_rVd(M_cut)_bestfit.png',dpi=300,bbox_inches="tight")
plt.show()
...
plt.title('$_{\mathrm{X}}-Y_{\mathrm{SZ}}$ : 1$\sigma$ & 3$\sigma$ contours for relaxed-disturbed')
plt.savefig('/home/schubham/Thesis/Thesis/Plots/Contour_plots/Lx-Ysz_relaxed_contours.png' ,dpi=300, bbox_inches="tight")
plt.show()
