...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 17 14:02:46 2021
...
norm_diff_obs = np.array(norm_diff_obs)
slope_diff_obs = np.array(slope_diff_obs)
scatter_diff_obs = np.array(scatter_diff_obs)
errnorm_diff_obs = np.array(errnorm_diff_obs)
errslope_diff_obs = np.array(errslope_diff_obs)
errscatter_diff_obs = np.array(errscatter_diff_obs)
best_norm_Clusters = np.array(best_norm_Clusters)
best_slope_Clusters = np.array(best_slope_Clusters)
err_slope_Clusters = np.array(err_slope_Clusters)
...
slope_Lx_T = np.array(slope_Lx_T)
slope_Ysz_T = np.array(slope_Ysz_T)
slope_Lx_Ysz = np.array(slope_Lx_Ysz)
slope_Lx_Lbcg = np.array(slope_Lx_Lbcg)
slope_Ysz_Lbcg = np.array(slope_Ysz_Lbcg)
slope_Lbcg_T = np.array(slope_Lbcg_T)
slope_R_Lx = np.array(slope_R_Lx)
slope_R_Ysz = np.array(slope_R_Ysz)
slope_R_Lbcg = np.array(slope_R_Lbcg)
slope_R_T = np.array(slope_R_T)


errslope_Lx_T = np.array(errslope_Lx_T )
errslope_Ysz_T = np.array(errslope_Ysz_T )
errslope_Lx_Ysz = np.array(errslope_Lx_Ysz )
errslope_Lx_Lbcg = np.array(errslope_Lx_Lbcg )
errslope_Ysz_Lbcg = np.array(errslope_Ysz_Lbcg )
errslope_Lbcg_T = np.array(errslope_Lbcg_T )
errslope_R_Lx = np.array(errslope_R_Lx )
errslope_R_Ysz = np.array(errslope_R_Ysz )
errslope_R_Lbcg = np.array(errslope_R_Lbcg )
errslope_R_T = np.array(errslope_R_T )

start = -100
end = 100
step = 3
del_Lx_range = np.arange(-80,100+step,step, dtype = 'int16') #15-160
del_T_range = np.arange(-50,50+step,step, dtype = 'int16')
del_Ysz_range = np.arange(-60,100+step,step, dtype = 'int16')
del_R_range = np.arange(-100,-20+step,step, dtype = 'int16')
del_Lbcg_range = np.arange(-100,50+step,step, dtype = 'int16')



final = np.array(np.meshgrid(del_Lx_range, del_T_range, del_Ysz_range, del_R_range, del_Lbcg_range)).T.reshape(-1,5)
...
del_Lx = del_Lx_range/100
del_T = 1 + (del_T_range/100)
...
errNorm_diff_exp_Lx_T = np.abs(Norm_diff_exp_Lx_T) * np.abs(np.log(del_T) * errslope_Lx_T) 

del_Ysz = del_Ysz_range/100
del_T = 1 + (del_T_range/100)
del_T_tot = (del_T ** slope_Ysz_T)-1
Norm_diff_exp_Ysz_T = (del_Ysz - del_T_tot) * 100
errNorm_diff_Ysz_T = np.abs(Norm_diff_exp_Ysz_T) * np.abs(np.log(del_T) * errslope_Ysz_T) 

del_Lx = del_Lx_range/100
del_Ysz = 1 + (del_Ysz_range/100)
del_Ysz_tot =( del_Ysz ** slope_Lx_Ysz) - 1
Norm_diff_exp_Lx_Ysz = (del_Lx - del_Ysz_tot) * 100
errNorm_diff_exp_Lx_Ysz = np.abs(Norm_diff_exp_Lx_Ysz) * np.abs(np.log(del_Ysz) * errslope_Lx_Ysz) 

del_R = del_R_range/100
del_Lx = 1 + (del_Lx_range/100)
del_Lx_tot = (del_Lx ** slope_R_Lx) - 1
Norm_diff_exp_R_Lx = (del_R - del_Lx_tot) * 100
errNorm_diff_exp_R_Lx = np.abs(Norm_diff_exp_R_Lx) * np.abs(np.log(del_Lx) * errslope_R_Lx) 

del_R = del_R_range/100
del_T = 1+ (del_T_range/100)
del_T_tot = (del_T ** slope_R_T) - 1
Norm_diff_exp_R_T = (del_R - del_T_tot) * 100
errNorm_diff_exp_R_T = np.abs(Norm_diff_exp_R_T) * np.abs(np.log(del_T) * errslope_R_T) 

del_R = del_R_range/100
del_Ysz = 1 + (del_Ysz_range/100)
del_Ysz_tot = (del_Ysz ** slope_R_Ysz) - 1
Norm_diff_exp_R_Ysz = (del_R - del_Ysz_tot) * 100
errNorm_diff_exp_R_Ysz = np.abs(Norm_diff_exp_R_Ysz) * np.abs(np.log(del_Ysz) * errslope_R_Ysz) 


del_Lx = del_Lx_range/100
del_Lbcg = 1 + (del_Lbcg_range/100)
del_Lbcg_tot = (del_Lbcg ** slope_Lx_Lbcg) - 1
Norm_diff_exp_Lx_Lbcg = (del_Lx - del_Lbcg_tot) * 100
errNorm_diff_exp_Lx_Lbcg = np.abs(Norm_diff_exp_Lx_Lbcg) * np.abs(np.log(del_Lbcg) * errslope_Lx_Lbcg) 

del_Ysz = del_Ysz_range/100
del_Lbcg = 1 + (del_Lbcg_range/100)
del_Lbcg_tot = (del_Lbcg ** slope_Ysz_Lbcg) - 1
Norm_diff_exp_Ysz_Lbcg = (del_Ysz - del_Lbcg_tot) * 100
errNorm_diff_exp_Ysz_Lbcg = np.abs(Norm_diff_exp_Ysz_Lbcg) * np.abs(np.log(del_Lbcg) * errslope_Ysz_Lbcg) 

del_Lbcg = del_Lbcg_range/100
del_T = 1 + (del_T_range/100)
del_T_tot = (del_T ** slope_Lbcg_T) - 1
Norm_diff_exp_Lbcg_T = (del_Lbcg - del_T_tot) * 100
errNorm_diff_exp_Lbcg_T = np.abs(Norm_diff_exp_Lbcg_T) * np.abs(np.log(del_T) * errslope_Lbcg_T) 

del_R = del_R_range/100
del_Lbcg = 1 + (del_Lbcg_range/100)
del_Lbcg_tot = (del_Lbcg ** slope_R_Lbcg) - 1
Norm_diff_exp_R_Lbcg = (del_R - del_Lbcg_tot) * 100
errNorm_diff_exp_R_Lbcg = np.abs(Norm_diff_exp_R_Lbcg) * np.abs(np.log(del_Lbcg) * errslope_R_Lbcg)
        ...
        for k in del_Ysz_range:
            for l in del_R_range:
                for m in del_Lbcg_range:
                    ...
                    errnorm_diff_exp_Ysz_T = np.abs(norm_diff_exp_Ysz_T) * np.abs(np.log(del_T) * errslope_Ysz_T) 
...
bestfit_bootstrap.to_csv('percent_diff_clusters_accepted_ccVncc_Clusters(step_3,Lx_0_40)_test.csv')
